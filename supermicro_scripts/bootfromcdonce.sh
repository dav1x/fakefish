...
# /redfish/v1/Systems/System_0/BootSourceOverrideEnabled 
#
curl -X PATCH -s -k -u ''"${BMC_USERNAME}"'':''"${BMC_PASSWORD}"'' https://${BMC_ENDPOINT}/redfish/v1/Systems/System_0 --data '{ "Boot":{"BootSourceOverrideEnabled":"Once","BootSourceOverrideMode":"UEFI","BootSourceOverrideTarget": "CD"}}'
if [ $? -eq 0 ]; then
  exit 0
else
  exit 1
